...
# Ablation study
#  ABLATION 1 — Parameterised model builder with swappable fusion mechanism
#  Reuses gelu / residual_block / dense_res_block / channel_attention /
#  adaptive_filter_capsule already defined earlier in the notebook.

def build_our_model_net_ablation(
    num_classes: int = 46,
    image_size: int = 32,
    head_units: int = 256,
    fusion_type: str = "concat_mlp",   # {"gated","fixed_sum","concat_mlp","dense_only","afc_only"}
    name: str = None,
) -> Model:
    """
    Variant of build_our_model_net used ONLY to ablate the final fusion
    mechanism. The backbone (stem + 3-stage encoder + multi-scale GAP) is
    byte-for-byte identical to the original; only the head differs.
    """
    assert fusion_type in {"gated", "fixed_sum", "concat_mlp", "dense_only", "afc_only"}
    K = num_classes
    inp = Input(shape=(image_size, image_size, 1), name="input")

    # ---- Stem (identical to original) ----
    t = layers.Conv2D(32, 3, padding="same", use_bias=False)(inp)
    t = layers.BatchNormalization()(t)
    t = layers.Activation(gelu)(t)
    ...
    # ---- Fusion mechanisms under test ----
    if fusion_type == "dense_only":
        outputs = x

    elif fusion_type == "afc_only":
        outputs = afc_out

    elif fusion_type == "fixed_sum":
        # No learnable parameters in the fusion step itself — equal blend.
        outputs = layers.Lambda(
            lambda t: 0.5 * t[0] + 0.5 * t[1], name="fixed_sum_fusion"
        )([x, afc_out])

    elif fusion_type == "concat_mlp":
        # Learnable fusion, but via a dense projection instead of a gate —
        # tests whether the *gating* structure matters or just having
        # learnable fusion weights is enough.
        combined = layers.Concatenate(name="concat_fusion_input")([x, afc_out])
        outputs = layers.Dense(num_classes, name="concat_fusion_logits")(combined)

    else:  # "gated" — original mechanism
        combined = layers.Concatenate(name="gate_input")([x, afc_out])
    ...
    model_name = name or f"our_model-Net-{fusion_type}"
    return keras.Model(inputs=inp, outputs=outputs, name=model_name)


# Registry of ablation variants — reuses the same train/eval loop pattern
# as MODELS_TF above.
MODELS_TF_ABLATION = {
    # "Gated (baseline)": lambda: build_our_model_net_ablation(NUM_CLASSES, IMG, fusion_type="gated"),
    # "Fixed 0.5/0.5 sum": lambda: build_our_model_net_ablation(NUM_CLASSES, IMG, fusion_type="fixed_sum"),
    "Concat + MLP fusion": lambda: build_our_model_net_ablation(NUM_CLASSES, IMG, fusion_type="concat_mlp"),
    # "Dense head only": lambda: build_our_model_net_ablation(NUM_CLASSES, IMG, fusion_type="dense_only"),
    # "AFC only": lambda: build_our_model_net_ablation(NUM_CLASSES, IMG, fusion_type="afc_only"),
}
...
#  ABLATION 2 — Train + evaluate every fusion variant
#  Mirrors section 8/9 of the main script but loops over MODELS_TF_ABLATION.

trained_models_ablation = {}
all_histories_ablation  = {}
ablation_results        = {}

# Reuse the already-computed steps_per_epoch / total_steps if present,
# otherwise recompute (cheap: just counts batches once).
try:
    steps_per_epoch
    total_steps
except NameError:
...
print(_c(f"\n{'═'*70}", "cyan"))
print(_c(f"  Gated-Fusion Ablation: {len(MODELS_TF_ABLATION)} variants × {CFG['epochs']} epochs", "bold", "white"))
print(_c(f"{'═'*70}\n", "cyan"))

for variant_name, model_fn in MODELS_TF_ABLATION.items():
    model = model_fn()
    model = compile_model(model, total_steps)

    print_model_summary(model)

    ckpt_path = os.path.join(CFG["results_dir"], f"ablation_{variant_name.replace(' ', '_')}_best.keras")
    callbacks = [
    ...
    print(f"\n{_c('  ▶ Training:', 'bold', 'cyan')} {_c(variant_name, 'yellow')}")
    ...
    trained_models_ablation[variant_name] = model
    all_histories_ablation[variant_name]  = history.history

    # ---- Test-set evaluation ----
    test_loss, test_acc_raw = model.evaluate(test_ds_oh, verbose=0)
    macro_f1 = compute_macro_f1(model, test_ds)
    ablation_results[variant_name] = {
        "test_acc":  round(test_acc_raw * 100.0, 2),
        "macro_f1":  round(macro_f1, 2),
        "params":    model.count_params(),
        "test_loss": round(float(test_loss), 4),
    }
...
#  ABLATION 3 — Inspect what the learned gate actually does
#  Only meaningful for the "Gated (baseline)" variant: pulls the per-sample
#  gate weights on the test set to see how much the model trusts the dense
#  head vs. the AFC branch, and whether that varies by class.

def analyze_gate_weights(model: Model, dataset, num_classes: int):
    """
    Runs the test set through the gated model and records the softmax gate
    output [w_dense, w_afc] for every sample. Requires the model to have a
    layer named "gate" (as produced by fusion_type="gated").
    """
    gate_layer_names = [l.name for l in model.layers if l.name == "gate"]
    if not gate_layer_names:
        print("[WARN] Model has no 'gate' layer — skipping (not the gated variant).")
        return None

    gate_probe = keras.Model(inputs=model.input, outputs=model.get_layer("gate").output)

    all_gates, all_labels = [], []
    for images, labels in dataset:
        gates = gate_probe(images, training=False).numpy()
        all_gates.append(gates)
        all_labels.append(labels.numpy())

    all_gates  = np.concatenate(all_gates,  axis=0)   # (N, 2)
    all_labels = np.concatenate(all_labels, axis=0)   # (N,)

    mean_w_dense, mean_w_afc = all_gates.mean(axis=0)
    print(f"[INFO] Mean gate weight — dense head: {mean_w_dense:.3f}  |  AFC: {mean_w_afc:.3f}")

    per_class_afc_weight = np.array([
        all_gates[all_labels == c, 1].mean() if np.any(all_labels == c) else np.nan
        for c in range(num_classes)
    ])
    top_afc_classes   = np.argsort(-per_class_afc_weight)[:5]
    top_dense_classes = np.argsort(per_class_afc_weight)[:5]
    print(f"[INFO] Classes relying most on AFC:   {top_afc_classes.tolist()}")
    print(f"[INFO] Classes relying most on dense: {top_dense_classes.tolist()}")

    return {"gates": all_gates, "labels": all_labels, "per_class_afc_weight": per_class_afc_weight}


if "Gated (baseline)" in trained_models_ablation:
    gate_analysis = analyze_gate_weights(
        trained_models_ablation["Gated (baseline)"], test_ds, NUM_CLASSES
    )

#  ABLATION 4 — Persist ablation results

ablation_results_path = os.path.join(CFG["results_dir"], "gated_fusion_ablation_results.json")
with open(ablation_results_path, "w") as f:
    json.dump(ablation_results, f, indent=2)
print(f"[INFO] Ablation results  → {ablation_results_path}")

ablation_histories_path = os.path.join(CFG["results_dir"], "gated_fusion_ablation_histories.json")
with open(ablation_histories_path, "w") as f:
    json.dump(
        {n: {k: [float(v) for v in vals] for k, vals in h.items()}
         for n, h in all_histories_ablation.items()},
        f, indent=2,
    )
print(f"[INFO] Ablation histories → {ablation_histories_path}")
print(_c("\n[DONE] Gated-fusion ablation study complete.\n", "green", "bold"))
