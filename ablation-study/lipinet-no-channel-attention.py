...
zip_path = "/kaggle/input/datasets/rautranjit/devanagari/DevanagariHandwrittenCharacterDataset"
...
# Ablation: our_model-Net without SE (channel attention) block
def build_our_model_net_no_se(num_classes: int = 46, image_size: int = 32,
    drop_path_rate: float = 0.05,
    dropout_rate: float = 0.3,
    weight_decay: float = 1e-4,
    head_units: int = 256,
    override_tier: int = None,) -> Model:
    """
    Ablation variant of our_model-Net with the SE (channel attention)
    block removed from the stem.

    Everything else (dual-path stem, dense residual encoder, scaffold
    injection, multi-scale GAP fusion, AFC, gated fusion head) is kept
    IDENTICAL to build_our_model_net so that any accuracy difference can
    be attributed solely to the absence of SE re-weighting.
    """
    K = num_classes
    ...
    model = keras.Model(inputs=inp, outputs=outputs, name="our_model-Net_noSE")
    return model


# Register both models so section 8's training loop runs the ablation
# comparison automatically and prints results side by side.
MODELS_TF = {
    # "our_model-Net":       lambda: build_our_model_net(NUM_CLASSES, IMG),
    "our_model-Net_noSE":  lambda: build_our_model_net_no_se(NUM_CLASSES, IMG),
}
