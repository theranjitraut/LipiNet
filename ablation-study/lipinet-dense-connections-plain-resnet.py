...
#  4b. ABLATION: PLAIN RESNET BLOCK (no dense/concat connections)

def plain_res_block(x, in_channels: int, out_channels: int):
    """
    Ablation variant of `dense_res_block` — plain ResNet block.

    Identical scaffolding (optional channel projection + strided
    depthwise downsampling) but the two residual sub-blocks are simply
    chained in sequence — NO concatenation / dense connection between
    r1 and r2. This isolates the contribution of the dense connections
    in the full WhatNet architecture.
    """
    # Optional projection (same as dense_res_block) 
    if in_channels != out_channels:
        skip = layers.Conv2D(out_channels, 1, use_bias=False)(x)
        skip = layers.BatchNormalization()(skip)
        x_in = layers.Activation(gelu)(skip)
    ...
    # Two chained residual blocks, NO dense/concat connection 
    r1 = residual_block(x_in, out_channels)
    out = residual_block(r1, out_channels)   # plain sequential stacking

    # Spatial downsampling via stride-2 depthwise conv (unchanged) 
    out = layers.DepthwiseConv2D(3, strides=2, padding="same", use_bias=False)(out)
    out = layers.Conv2D(out_channels, 1, use_bias=False)(out)
    out = layers.BatchNormalization()(out)
    out = layers.Activation(gelu)(out)
...
def build_plain_resnet_ablation(num_classes: int = 46, image_size: int = 32,
    head_units: int = 256,) -> Model:
    """
    Ablation model: identical to our_model-Net EXCEPT dense_res_block
    is replaced by plain_res_block (no dense connections).
    Everything else — stem, scaffold injection, multi-scale GAP fusion,
    AFC, gated fusion head — is kept exactly the same, so any accuracy
    delta vs. the full model can be attributed to the dense connections.
    """
    K = num_classes
    inp = Input(shape=(image_size, image_size, 1), name="input")
    ...
    # Encoder — plain_res_block instead of dense_res_block
    enc1 = plain_res_block(stem, 64, 64)
    sc1  = layers.AveragePooling2D(2)(layers.Conv2D(64, 1, use_bias=False)(scaffold))
    enc1 = layers.Add()([enc1, layers.Lambda(lambda t: t * 0.1)(sc1)])

    enc2 = plain_res_block(enc1, 64, 128)
    sc2  = layers.AveragePooling2D(4)(layers.Conv2D(128, 1, use_bias=False)(scaffold))
    enc2 = layers.Add()([enc2, layers.Lambda(lambda t: t * 0.1)(sc2)])

    enc3 = plain_res_block(enc2, 128, 256)
    sc3  = layers.AveragePooling2D(8)(layers.Conv2D(256, 1, use_bias=False)(scaffold))
    enc3 = layers.Add()([enc3, layers.Lambda(lambda t: t * 0.1)(sc3)])

    # Multi-scale GAP fusion (unchanged)
    gap1 = layers.GlobalAveragePooling2D(name="gap1")(enc1)
    gap2 = layers.GlobalAveragePooling2D(name="gap2")(enc2)
    gap3 = layers.GlobalAveragePooling2D(name="gap3")(enc3)
    fused_gap = layers.Concatenate(name="multiscale_fused")([gap1, gap2, gap3])

    # AFC + gated fusion head (unchanged)
    ...
    model = keras.Model(inputs=inp, outputs=outputs, name="Ablation-PlainResNet")
    return model


# Add to the model registry so it trains/evaluates alongside the full model
MODELS_TF = {
    "Ablation-PlainResNet": lambda: build_plain_resnet_ablation(NUM_CLASSES, IMG)
}
    # MODELS_TF = {
#     "our_model-Net":    lambda: build_our_model_net(NUM_CLASSES, IMG),
