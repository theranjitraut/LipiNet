...
'''
### Ablation Study: Stroke Scaffold Kernel Shape

The stem's scaffold path uses a `1×5` **horizontal asymmetric convolution**
to target the `shirorekha` (top bar) and other horizontal strokes in
Devanagari glyphs. The kernel width (5) and its "horizontal-only" shape
(height=1) were both hand-picked. This section turns the kernel into a
hyperparameter and sweeps:

* **Horizontal widths**: `1×3`, `1×5`, `1×7`, `1×9`, `1×11` — does a wider
  receptive field along the shirorekha help, or does it just blur things?
* **Controls**: `3×3` (isotropic, i.e. no horizontal bias at all) and `5×1`
  (vertical asymmetric — the "wrong" orientation) to confirm that the
  horizontal prior is actually earning its keep and it isn't simply
  "more parameters = better."

Everything else (scaffold_weight=0.1, encoder, head) is held fixed at the
base-model values so the kernel shape is the only thing changing.
'''
# Scaffold kernel shape as a hyperparameter
# Identical to build_our_model_net(), except the previously hard-coded
# `(1, 5)` scaffold-conv kernel is now a function argument so it can be
# swept for the kernel-shape ablation. `scaffold_weight` is held fixed at
# the base-model value (0.1) so kernel shape is the only variable.

def build_our_model_net_kernel_ablation(
    num_classes: int = 46,
    image_size: int = 32,
    scaffold_kernel: tuple = (1, 5),
    scaffold_weight: float = 0.1,
) -> Model:
    """our_model-Net with a configurable stroke-scaffold kernel shape."""
    inp = Input(shape=(image_size, image_size, 1), name="input")
    ...
    # Stem — stroke scaffold path (kernel shape under test)
    s = layers.Conv2D(32, scaffold_kernel, padding="same", use_bias=False)(inp)
    s = layers.BatchNormalization()(s)
    scaffold = layers.Activation(gelu)(s)
    ...
    def scaled(tensor, w):
        # capture w by default-arg to avoid late-binding closure bugs
        return layers.Lambda(lambda x, w=w: x * w)(tensor)

    # Encoder stage 1
    enc1 = dense_res_block(stem, 64, 64)
    sc1  = layers.AveragePooling2D(2)(layers.Conv2D(64, 1, use_bias=False)(scaffold))
    enc1 = layers.Add()([enc1, scaled(sc1, scaffold_weight)])

    # Encoder stage 2
    enc2 = dense_res_block(enc1, 64, 128)
    sc2  = layers.AveragePooling2D(4)(layers.Conv2D(128, 1, use_bias=False)(scaffold))
    enc2 = layers.Add()([enc2, scaled(sc2, scaffold_weight)])

    # Encoder stage 3
    enc3 = dense_res_block(enc2, 128, 256)
    sc3  = layers.AveragePooling2D(8)(layers.Conv2D(256, 1, use_bias=False)(scaffold))
    enc3 = layers.Add()([enc3, scaled(sc3, scaffold_weight)])
    ...
    afc_out = adaptive_filter_capsule(fused_gap, num_classes)
    ...
    # Gated fusion of dense head + AFC
    combined = layers.Concatenate(name="gate_input")([x, afc_out])
    gate = layers.Dense(2, activation="softmax", name="gate")(combined)
    x_scaled   = layers.Lambda(lambda t: t[0] * t[1][:, 0:1], name="gate_dense")([x, gate])
    afc_scaled = layers.Lambda(lambda t: t[0] * t[1][:, 1:2], name="gate_afc")([afc_out, gate])
    outputs = layers.Add(name="logits")([x_scaled, afc_scaled])

    kh, kw = scaffold_kernel
    tag = f"{kh}x{kw}"
    model = keras.Model(inputs=inp, outputs=outputs, name=f"our_model-Net_k{tag}")
    return model

# Add / remove tuples here to change what gets tested.
# (kh, kw): kh=1 -> horizontal asymmetric strip, kw=1 -> vertical asymmetric
# strip, kh==kw -> isotropic square control.
#
# "3x3"  -> isotropic control (no directional bias at all)
# "1x3"  -> narrow horizontal strip
# "1x5"  -> BASE MODEL setting
# "1x7"  -> wider horizontal strip
# "1x9"  -> wider still
# "1x11" -> very wide horizontal strip
# "5x1"  -> vertical control (wrong orientation, same param count as 1x5)

SCAFFOLD_KERNELS = [
    (3, 3),   # isotropic control
    (1, 3),
    (1, 5),   # base model
    (1, 7),
    (1, 9),
    (1, 11),
    (5, 1),   # vertical control (same #params as 1x5)
]

KERNEL_ABLATION_EPOCHS   = 100    # same budget as the scaffold-weight ablation
KERNEL_ABLATION_PATIENCE = 15     # early-stopping patience (on val_accuracy)

kernel_ablation_results   = {}
kernel_ablation_histories = {}

steps_per_epoch_kabl = sum(1 for _ in train_ds)
total_steps_kabl      = steps_per_epoch_kabl * KERNEL_ABLATION_EPOCHS

print(_c(f"\n{'='*70}", "cyan"))
print(_c(f"  Scaffold-kernel ablation: {len(SCAFFOLD_KERNELS)} runs x {KERNEL_ABLATION_EPOCHS} epochs", "bold", "white"))
print(_c(f"{'='*70}\n", "cyan"))

for k in SCAFFOLD_KERNELS:
    kh, kw = k
    tag = f"kernel={kh}x{kw}"
    keras.backend.clear_session()

    model = build_our_model_net_kernel_ablation(NUM_CLASSES, IMG, scaffold_kernel=k)
    model = compile_model(model, total_steps_kabl)

    print(f"\n{_c('  Training:', 'bold', 'cyan')} {_c(tag, 'yellow')}")

    ckpt_path = os.path.join(CFG["results_dir"], f"ablation_kernel_{kh}x{kw}_best.keras")
    callbacks = [
        ModelCheckpoint(ckpt_path, monitor="val_accuracy", save_best_only=True, verbose=0),
        EarlyStopping(monitor="val_accuracy", patience=KERNEL_ABLATION_PATIENCE,
                       restore_best_weights=True, verbose=0),
    ]
    ...
    kernel_ablation_results[tag] = {
        "kernel":        f"{kh}x{kw}",
        "kernel_h":      kh,
        "kernel_w":      kw,
        "is_horizontal": kh == 1 and kw > 1,
        "is_vertical":   kw == 1 and kh > 1,
        "is_isotropic":  kh == kw,
        "val_acc_best":  round(best_val, 2),
        "test_acc":      round(test_acc, 2),
        "macro_f1":      round(macro_f1, 2),
        "test_loss":     round(float(test_loss), 4),
        "params":        model.count_params(),
    ...
    kernel_ablation_histories[tag] = history.history

    print(
        f"  {_c('✔ Done:', 'green', 'bold')} "
        f"test_acc={_c(f'{test_acc:.2f}%', 'green')}  "
...
# Results
def print_kernel_ablation_table(results: dict) -> None:
    best_tag = max(results, key=lambda k: results[k]["test_acc"])
    print(f"\n{_c('=== SCAFFOLD-KERNEL ABLATION - TEST-SET COMPARISON ===', 'cyan', 'bold')}")
    hdr = (f"{'Kernel':>8} | {'Type':>11} | {'Val Acc':>8} | {'Test Acc':>9} | "
           f"{'Macro F1':>9} | {'Loss':>7} | {'Params':>10} | {'Time(s)':>8}")
    print(_c(hdr, "bold", "white"))
    print("-" * len(hdr))
    ...
    # Sort by kernel width so the horizontal sweep reads left-to-right,
    # with the two controls (3x3, 5x1) falling out naturally by kh/kw.
    for tag, r in sorted(results.items(), key=lambda kv: (kv[1]["kernel_h"], kv[1]["kernel_w"])):
        star = "*" if tag == best_tag else " "
        row = (
            f"{star}{r['kernel']:>7} | {kind(r):>11} | {r['val_acc_best']:>7.2f}% | "
            f"{r['test_acc']:>8.2f}% | {r['macro_f1']:>8.2f}% | "
            f"{r['test_loss']:>7.4f} | {r['params']:>10,} | {r['train_time_s']:>8.1f}"
        )
        print(_c(row, "green", "bold") if tag == best_tag else row)

    best = results[best_tag]
    print(_c(
        f"\n  Best scaffold kernel = {best['kernel']}  "
        f"(test_acc={best['test_acc']:.2f}%, macro_f1={best['macro_f1']:.2f}%)\n",
        "green", "bold",
    ))

print_kernel_ablation_table(kernel_ablation_results)

# Persist results + histories
kernel_ablation_results_path = os.path.join(CFG["results_dir"], "ablation_scaffold_kernel_results.json")
with open(kernel_ablation_results_path, "w") as f:
    json.dump(kernel_ablation_results, f, indent=2)
print(f"Kernel-ablation results -> {kernel_ablation_results_path}")

kernel_ablation_histories_path = os.path.join(CFG["results_dir"], "ablation_scaffold_kernel_histories.json")
with open(kernel_ablation_histories_path, "w") as f:
    json.dump(
        {tag: {k: [float(v) for v in vals] for k, vals in h.items()}
         for tag, h in kernel_ablation_histories.items()},
        f, indent=2,
    )
print(f"[INFO] Kernel-ablation histories -> {kernel_ablation_histories_path}")

# Plot: test accuracy & macro-F1 vs horizontal kernel width, controls shown separately
import matplotlib.pyplot as plt

horiz = sorted(
    [r for r in kernel_ablation_results.values() if r["is_horizontal"]],
    key=lambda r: r["kernel_w"],
)
controls = [r for r in kernel_ablation_results.values() if not r["is_horizontal"]]

widths      = [r["kernel_w"] for r in horiz]
test_accs   = [r["test_acc"] for r in horiz]
macro_f1s   = [r["macro_f1"] for r in horiz]

fig, ax = plt.subplots(figsize=(8, 5))
ax.plot(widths, test_accs, marker="o", label="Test Accuracy (1×k horizontal)")
ax.plot(widths, macro_f1s, marker="s", label="Macro F1 (1×k horizontal)")

# Overlay the isotropic / vertical controls as horizontal reference lines
for r in controls:
    label = f"{r['kernel']} ({'isotropic' if r['is_isotropic'] else 'vertical'}) test_acc"
    ax.axhline(r["test_acc"], linestyle="--", alpha=0.6, label=label)

ax.set_xlabel("Horizontal scaffold kernel width (k in 1×k)")
ax.set_ylabel("Score (%)")
ax.set_title("Scaffold Kernel Shape Ablation — Devanagari")
ax.legend(fontsize=8)
ax.grid(alpha=0.3)
plt.tight_layout()
plot_path = os.path.join(CFG["results_dir"], "ablation_scaffold_kernel_plot.png")
plt.savefig(plot_path, dpi=150)
plt.show()
print(f"[INFO] Plot saved -> {plot_path}")

print(_c(f"\n{'='*70}", "cyan"))
print(_c(f"  How to read this ablation:", "bold", "white"))
print(_c(f"{'='*70}", "cyan"))